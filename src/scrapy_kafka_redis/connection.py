...
def get_request_consumer_from_settings(spider_name: str, settings: Settings) -> KafkaConsumer:
    bootstrap_servers = settings.get('KAFKA_BOOTSTRAP_SERVERS', defaults.KAFKA_BOOTSTRAP_SERVERS)
    kafka_params = settings.get('KAFKA_REQUEST_CONSUMER_PARAMS', defaults.KAFKA_REQUEST_CONSUMER_PARAMS)
    kafka_request_group = settings.get('KAFKA_REQUEST_GROUP', defaults.KAFKA_REQUEST_GROUP)
    kafka_params['group_id'] = kafka_request_group % {'spider': spider_name}
    consumer = KafkaConsumer(bootstrap_servers=bootstrap_servers, **kafka_params)
    return consumer
...
def get_request_producer_from_settings(settings: Settings) -> KafkaProducer:
    bootstrap_servers = settings.get('KAFKA_BOOTSTRAP_SERVERS', defaults.KAFKA_BOOTSTRAP_SERVERS)
    kafka_params = settings.get('KAFKA_REQUEST_PRODUCER_PARAMS', defaults.KAFKA_REQUEST_PRODUCER_PARAMS)
